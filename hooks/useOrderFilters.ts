  ...
  const filteredOrders = (() => {
    ...
    if (debouncedSearch) {
      ...
      filtered = filtered.filter((order) => {
        const hasMatchingOrderNo = String(order.order_no)
          .toLowerCase()
          .includes(query);
    ...
    if (filters.selectedStatus && filters.selectedStatus !== "ALL") {
      filtered = filtered.filter((order) =>
        order.tenant_orders.some(
          (to: any) => to.order_status?.value === filters.selectedStatus,
        ),
      );
    }
    ...
    if (filters.dateFrom || filters.dateTo) {
      filtered = filtered.filter((order) => {
        ...
        if (filters.dateFrom && filters.dateTo) {
          return (
            orderDate >= startOfDay(filters.dateFrom) &&
            orderDate <= endOfDay(filters.dateTo)
          );
        }
        if (filters.dateFrom) {
